...
const getToken = (type) => {
    let o = (exec('docker swarm join-token ' + type, {stdio: [null, 'ignore', 'ignore']}) || '')['toString']('utf8');
...
let stopContainer = (name) => {
    ...
    try {
        res = exec(`docker stop $(docker ps -aq --filter="name=${name}")`, {stdio: [null, 'ignore', 'ignore']});
    } catch (e) {
...
let rmContainer = (name) => {
    ...
    try {
        res = exec(`docker rm -v $(docker ps -aq --filter="name=${name}")`, {stdio: [null, 'ignore', 'ignore']});
    } catch (e) {
        console.log(e.stderr['toString']('utf8'));
        if (e.stderr['toString']('utf8').indexOf('requires at least 1 argument.') > 0) {
            console.info('FOUND NOW', name);
